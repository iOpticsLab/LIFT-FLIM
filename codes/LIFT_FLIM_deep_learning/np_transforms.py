...
class RectCrop(object):
    ...
    def __init__(self, rect_pos):
        # rect_pos: [top, left, height, width]
        self.corner_pos = rect_pos[:2]
        self.size = rect_pos[2:]
    ...
    @staticmethod
    def get_params(corner_pos, output_size):
        """Get parameters for ``crop`` for center crop.
        Args:
            pic (np array): Image to be cropped.
            output_size (tuple): Expected output size of the crop.
    ...
    def __call__(self, pic):
        ...
        # get crop params: starting pixels and size of the crop
        i, j, h, w = self.get_params(self.corner_pos, self.size)
        ...
        if isinstance(self.size, int):
            ...
            if len(pic.shape) == 3:
                ...
                for i in range(pic.shape[2]):
                    img_out[:, :, i] = ndimage.zoom(pic[:, :, i], (ow, oh), order=1)
                return img_out
            else:
                # if 2D image, scale image
                return ndimage.zoom(pic, (ow, oh), order=1)
        else:
            # if size is specified with 2 dimensions apply the scale directly
            w, h = pic.shape[:2]
            ow, oh = self.size[0]/w, self.size[1]/h
