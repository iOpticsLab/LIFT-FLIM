...
class down_shifted_dwconv2d(nn.Module):
    def __init__(self, num_filters_in, num_filters_out, filter_size=(7,7), stride=(1,1), 
                    shift_output_down=False, norm='batch_norm'):
        super(down_shifted_dwconv2d, self).__init__()
        
        assert norm in [None, 'layer_norm', 'batch_norm', 'weight_norm']
        assert num_filters_out % num_filters_in == 0
        self.conv = nn.Sequential(nn.Conv2d(num_filters_in, num_filters_out, filter_size, stride, groups=num_filters_in),  # depthwise conv
                                    nn.Conv2d(num_filters_out, num_filters_out, (1,1)))  # pointwise conv
        self.shift_output_down = shift_output_down
        self.norm = norm
        self.pad  = nn.ZeroPad2d((int((filter_size[1] - 1) / 2), # pad left
                                  int((filter_size[1] - 1) / 2), # pad right
        ...
        if norm == 'weight_norm':
            ...
            self.bn = nn.LayerNorm(num_filters_out)
    ...
    def forward(self, x):
        x = self.pad(x)
        x = self.conv(x)
        x = self.bn(x) if (self.norm == 'batch_norm') or (self.norm == 'layer_norm') else x
        return self.down_shift(x) if self.shift_output_down else x
...
class down_right_shifted_dwconv2d(nn.Module):
    def __init__(self, num_filters_in, num_filters_out, filter_size=(7,7), stride=(1,1), 
                    shift_output_right=False, norm='batch_norm'):
        super(down_right_shifted_dwconv2d, self).__init__()
        
        assert norm in [None, 'layer_norm', 'batch_norm', 'weight_norm']
        assert num_filters_out % num_filters_in == 0
        self.pad = nn.ZeroPad2d((filter_size[1] - 1, 0, filter_size[0] - 1, 0))
        self.conv = nn.Sequential(nn.Conv2d(num_filters_in, num_filters_out, filter_size, stride=stride, groups=num_filters_in),  # depthwise conv
                                    nn.Conv2d(num_filters_out, num_filters_out, (1,1)))  # pointwise conv
        self.shift_output_right = shift_output_right
        self.norm = norm
...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0):
        ...
        if nonlinearity == 'concat_elu' : 
            self.activation = lambda x : concat_elu(x)
        ...
        if nonlinearity == 'concat_elu':
            ...
            self.conv_input = conv_op(num_filters, num_filters)
        
        if skip_connection != 0 and self.nonlinearity == 'concat_elu': 
            self.nin_skip = nin(2 * skip_connection * num_filters, num_filters)
        elif skip_connection != 0:
        ...
        if nonlinearity == 'concat_elu':
            ...
            self.conv_out = conv_op(num_filters, num_filters)


    def forward(self, og_x, a=None):
        x = self.conv_input(self.activation(og_x))
        if a is not None : 
            x += self.nin_skip(self.activation(a))
        x = self.activation(x)
        x = self.dropout(x)
        x = self.conv_out(x)
        if self.nonlinearity == 'concat_elu':
